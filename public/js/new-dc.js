	...
	function setScrollableContent(i){
		var tDate = moment(new Date()).format("YYYY-MM-DD")
				scrollableContent = "<tr class=\"product-details-row-scrollable\" row-index=\"\"> <td class=\"product-detail-cell\" id=\"dcSerial\"> <input type=\"text\" class=\"table-cell-input text-center\" id=\"dcSerial\" name=\"dcProducts["+i+"][dcSerial]\" value=\"\" readonly> </td> <td class=\"product-detail-cell\" id=\"description\"> <input type=\"text\" class=\"table-cell-input\" id=\"description\" name=\"dcProducts["+i+"][description]\" value=\"\"><a href=\"javascript:;\" id=\"search_product\"><i class=\"fa fa-search\"></i></a> </td><td class=\"product-detail-cell\" id=\"quantity\"> <input type=\"text\" class=\"table-cell-input\" id=\"quantity\" name=\"dcProducts["+i+"][quantity]\" value=\"1\"> </td> <td class=\"product-detail-cell\" id=\"unit\"> <select id=\"unit\" name=\"dcProducts["+i+"][unit]\" class=\"table-cell-input\"> <option value=\"NOS\">NOS</option><option value=\"BGS\">Bags</option><option value=\"Brass\">Brass</option><option value=\"BTL\">Bottles</option><option value=\"CAN\">Cans</option><option value=\"KG\">Kilograms</option><option value=\"LTR\">liter</option><option value=\"MTR\">Meter</option><option value=\"CH\">Chhota Hatti</option></select> </td><td class=\"product-detail-cell\"> <input type=\"date\" id=\"dcDate\" name=\"dcProducts["+i+"][dcDate]\"  style=\"width: 118px;\" value="+tDate+"> </td> <td class=\"product-detail-cell\" id=\"vehicleNo\"> <input type=\"text\" class=\" table-cell-input\" id=\"vehicleNo\" name=\"dcProducts["+i+"][vehicleNo]\" value=\"\" > </td> <td class=\"product-detail-cell\" id=\"dcNo\"> <input type=\"text\" class=\" table-cell-input\" id=\"dcNo\" name=\"dcProducts["+i+"][dcNo]\" value=\"\" > </td> </tr> ";
	...
	$("input#customerName").autocomplete({
		...
		source:[
			function( q,add ){
				...
				if( q !== "") {
					$.ajax({
						url: baseURL+"/dcs/select_customer/"+encodeURIComponent(q),
						dataType : 'json',
	...
	//**************     on select of a dc product in selct product modal     ********* */
	//************************************************************************************** */
	//************************************************************************************** */
	...
	$("input#searchProductDescription").autocomplete({
		...
		source:[
			function( q,add ){
				...
				if( q !== "") {
					$.ajax({
						url: baseURL+"/dcs/select_product/"+encodeURIComponent(q),
						dataType : 'json',
						success: function(resp){
							add(resp.data);
						}
	...
	function add_empty_productrow() {
		...
		setScrollableContent(dc_product_serial);

		setFixedContent(dc_product_serial);

		$(".table-body-scrollable table tbody .dc-totals-row-scrollable").before(scrollableContent);
		$(".table-body-fixed table tbody .dc-totals-row-fixed").before(fixedContent);
		$('.table-body-scrollable table tbody tr.product-details-row-scrollable:last').attr('row-index', dc_product_serial);
		$('.table-body-fixed table tbody tr.product-details-row-fixed:last').attr('row-index', dc_product_serial);
		$('input#dcSerial:last').val(dc_product_serial);
	...
	function renumber_productrow() {
		...
		$('input#dcSerial').each(function(i, obj) {
			$(this).val(i+1);
			dc_product_serial = i+2;
		});
	}
	...
	tableFixed.on('click', 'a.delete-row-link', function(e){
		...
		dc_product_serial = 1; // Since renumbering wont happen after deleting all rows in previous step, assinging value manually
		renumber_productrow();
		setTotalsRow();
		finalRoundingoff();
	});
	...
	$('select#dcStatus').on('change', function(e){
	...
	$(document).ready(function(){
		...
		$('.dc-products-list-table').hScroll(30);
	});
	...
	function setRowHeight() {
		$('tr.product-details-row-scrollable').each(function(i, obj) {
			var sourceElement = $(this).find('td#dcSerial');
	...
	function setTotalsRow() {
		var totalsRow = $('.dc-totals-row-scrollable');
	...
	function setInvoiceStatus(amountRecieved) {
		...
		$('select#dcStatus').children('option[selected]').removeAttr('selected');

		if(amountRecieved < 1){
			$('select#dcStatus').val("unpaid").change();
		}else if(amountRecieved < grandValue){
			$('select#dcStatus').val("partial").change();
		}else if(amountRecieved === grandValue){
			$('select#dcStatus').val("paid").change();
		}else if(amountRecieved > grandValue){
			$('select#dcStatus').val("paid").change();
		}
		// if(amountRecieved < 1){
		// 	$('select#dcStatus').children('option[value="unpaid"]').attr('selected', 'selected');
		// }else if(amountRecieved < $('input#grandValue').val()){
		// 	$('select#dcStatus').children('option[value="partial"]').attr('selected', 'selected');
		// }else if(amountRecieved === $('input#amountRecieved').val()){
		// 	$('select#dcStatus').children('option[value="paid"]').attr('selected', 'selected');
		// }
	}
